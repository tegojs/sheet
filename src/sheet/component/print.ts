...
export default class Print {
          ...
          h('div', '-sider').child(
            h('form', '').children(
              ...
              h('fieldset', '').children(
                ...
                h('select', '')
                  .children(
                    ...PAGER_MODEL.map((it, index) =>
                      h('option', '')
                        .attr('value', index)
                        .child(`${t('print.printmodels')[index]}`),
                    ),
                  )
              ...
              h('fieldset', '').children(
                ...
                h('select', '')
                  .children(
                    ...PAGER_ISPRINT.map((it, index) =>
                      h('option', '').attr('value', index).child(it),
                    ),
                  )
  ...
  previewScaleCols() {
    ...
    mViewRange.sci = 0;
    let yof = 0;
    pageDraws[0].clearRect(
      0,
      pageyoffsets[0],
      iwidth,
      iheight - pageyoffsets[0] + 1,
    );
    this.printFooter(
      pageDraws[0],
      iwidth / 2 - 30,
    ...
    for (let ipage = 1; ipage < pageDraws.length; ipage++) {
      ...
      mViewRange.eri =
        pageRowIndexs[ipage] == undefined ? cr.eri : pageRowIndexs[ipage];
      ...
      if (ipage < pageDraws.length - 1) {
        ...
        pageDraws[ipage].clearRect(
          0,
          pageyoffsets[ipage],
          iwidth,
          iheight - pageyoffsets[ipage] + 1,
        );
      }
      this.printFooter(
        pageDraws[ipage],
  ...
  previewScaleRows() {
    ...
    for (let ipage = 1; ipage < pageDraws.length; ipage++) {
      mViewRange.eci =
        pageColIndexs[ipage] == undefined ? cr.eci : pageColIndexs[ipage];
  ...
  previewNoScale() {
    ...
    for (let ipage = 1; ipage < pageDraws.length; ipage++) {
      ...
      if (colIndex < pageColIndexs.length) {
        mViewRange.eci =
          pageColIndexs[colIndex + 1] == undefined
            ? cr.eci
            : pageColIndexs[colIndex + 1];
        mViewRange.eri =
          pageRowIndexs[rowIndex] == undefined
            ? cr.eri
            : pageRowIndexs[rowIndex];
        mViewRange.sci = pageColIndexs[colIndex];
        xof -= pagexoffsets[colIndex];

        colIndex++;
      } else if (
        colIndex == pageColIndexs.length &&
        rowIndex < pageRowIndexs.length
      ) {
        //检测行
        mViewRange.sci = 0;
        mViewRange.eci =
          pageColIndexs[0] == undefined ? cr.eci : pageColIndexs[0];
        mViewRange.eri =
          pageRowIndexs[rowIndex + 1] == undefined
            ? cr.eri
            : pageRowIndexs[rowIndex + 1];
