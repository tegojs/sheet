...
export default class DataProxy {
  ...
  setSelectedCellAttr(property: string, value: unknown): void {
    this.changeData(() => {
      ...
      if (property === 'merge') {
        ...
        setStyleBorders.call(
          this,
          value as { mode: string; style: unknown; color: unknown },
        );
  ...
  rowEach(
    min: number,
    max: number,
    cb: (ri: number, y: number, height: number) => void,
  ): void {
    let y = 0;
    const { rows } = this;
    const frset = this.exceptRowSet;
  ...
  colEach(
    min: number,
    max: number,
    cb: (ci: number, x: number, width: number) => void,
  ): void {
    let x = 0;
    const { cols } = this;
